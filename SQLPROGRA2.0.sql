...
create table tareas (
    ...
    nota3 int,
    nota4 int,
    constraint FK_tareas_Tb_alumnos foreign key (Carnet) references Tb_alumnos(carnet)
);
...
foreign key (carnet) references Tb_alumnos(carnet)
);

insert into asistencia(carnet, fecha, presente) values
('0905-24-6913','2025-03-20','S'),
('0905-24-7000','2025-03-20','S'),
('0905-24-7010','2025-03-20','N'),
('0905-24-7370','2025-03-20','N'),
('0905-24-7854','2025-03-20','S');
...
select a.Estudiante, b.nota1, b.nota2, b.nota3, b.nota4
from Tb_alumnos a
join tareas b on a.carnet = b.carnet;


update notas_finales 
set nota_final = 85.00
where carnet notas_finales

select * from asistencia 

delete from asistencia
where Id_asistencia = 8;

with asistencia_reciente as(
select carnet, fecha, ROW_NUMBER() over (partition by carnet order by fecha desc) as rn
from asistencia
)
select a.estudiante, ar.fecha as asistencia_reciente, nf.nota_final
from Tb_alumnos a
join asistencia_reciente ar on a.carnet = ar.carnet and ar.rn = 1
